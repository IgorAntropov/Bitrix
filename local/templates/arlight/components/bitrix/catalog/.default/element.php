...
//подменяем код секции для товаров с двумя секциями для хлебных крошек
$sectionCode = $arResult['VARIABLES']['SECTION_CODE'];
//страница , с которой пришли
$HTTP_REFERER = $_SERVER['HTTP_REFERER'];
$REFERER_SECTION_CODE_AR = explode('/', trim($HTTP_REFERER, '/'));
if ($REFERER_SECTION_CODE_AR[3] == 'catalog') {
    $REFERER_SECTION_CODE = $REFERER_SECTION_CODE_AR[4];
    $elementCode = $arResult['VARIABLES']['ELEMENT_CODE'];
    $article = end(explode('-', $elementCode));
    $articlesToID = [];
    if (file_exists($_SERVER["DOCUMENT_ROOT"] . SITE_DIR . 'cron/catalog_import/data/json/product_ids.json'))
        $articlesToID = json_decode(json_encode(json_decode(file_get_contents($_SERVER["DOCUMENT_ROOT"] . SITE_DIR . 'cron/catalog_import/data/json/product_ids.json'))), true);
    $ID = $articlesToID[$article];

    if (!$ID) {
        $arFilter = array("IBLOCK_ID" => CATALOG_ID, "CODE" => $arResult['VARIABLES']['ELEMENT_CODE']);
        $res = CIBlockElement::GetList(array(), $arFilter, false, array("nPageSize" => 1), array('ID'));
        $element = $res->Fetch();
        $ID = $element['ID'];
    }

    //разделы, к которым относится товар
    $sectionArAllCode = [];
    $db_old_groups = CIBlockElement::GetElementGroups($ID, true, ['CODE', 'ID']);
    while ($ar_group = $db_old_groups->Fetch()) {
        $sectionAr[] = $ar_group['CODE'];
        $sectionArAllCode = array_merge($sectionArAllCode, GetIBlockSectionAllPath(CATALOG_ID, $ar_group['ID']));
    }

    //если товар в нескольких категориях, устанавливаем тот ,  из которого пришлии
    if (isset($sectionAr) && count($sectionAr) > 1 && in_array($REFERER_SECTION_CODE, $sectionAr)) {
        $sectionCode = $REFERER_SECTION_CODE;
    } else {
        /*если пришли не из 3го уровня*/
        function findMyCode($item)
        {
            global $REFERER_SECTION_CODE;
            return $item['CODE'] == $REFERER_SECTION_CODE;
        }

        $refererCodeAllInfo = array_filter($sectionArAllCode, "findMyCode");
        if (!empty($refererCodeAllInfo)) {
            $refererCodeAllInfoKey = array_keys($refererCodeAllInfo)[0];
            $refererCodeAllInfoValue = $refererCodeAllInfo[$refererCodeAllInfoKey];
            if ($refererCodeAllInfoValue['DEPTH_LEVEL'] == 1)
                $sectionCode = $sectionArAllCode[$refererCodeAllInfoKey + 2]['CODE'];
            if ($refererCodeAllInfoValue['DEPTH_LEVEL'] == 2)
                $sectionCode = $sectionArAllCode[$refererCodeAllInfoKey + 1]['CODE'];
        }
    }
}
?>
