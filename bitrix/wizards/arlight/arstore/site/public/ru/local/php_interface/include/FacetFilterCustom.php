...
class FacetMod extends Facet {
    public function query(array $filter, array $facetTypes = array(), $facetId = 0)
    {
        ...
        if ($filter)
        {
            ...
            //Вот тут кастомизируем, включая фильтр --- НАЧАЛО
            if(is_array($GLOBALS["obsoleteFilter"]))
                $filter=array_merge($filter,$GLOBALS["obsoleteFilter"]);
            //Вот тут кастомизируем, включая фильтр --- КОНЕЦ
